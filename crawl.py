...
import json
import datetime
from server import scrape_afa_news, scrape_moa_news, consolidate_and_save, scheduler_status
...
def main():
    ...
    # 合併並儲存
    all_news = afa_news + moa_news
    if all_news:
        print("[3/3] 正在整合新聞資料並寫入歷史檔案...")
        consolidate_and_save(all_news)
    else:
        print("[3/3] 無法取得任何新聞資料，跳過整合步驟。")
